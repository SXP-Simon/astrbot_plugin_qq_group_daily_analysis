...
增量分析按目标群新增消息数量触发。每个群独立累计待处理消息数，达到 `incremental_min_messages` 后立即处理一个固定规模批次。消息爆发时会连续拆分为多个批次，低活跃群则持续等待消息累积，不受时间段、执行间隔或每日次数限制。
...
6. 分析失败时保留待处理计数，不进行无间隔重试；下一条目标群消息到达或插件重启后再尝试。
