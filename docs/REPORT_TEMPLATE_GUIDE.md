...
- **预览图**：`assets/<模板名>-demo.jpg`（仓库根 `assets/` 下）。
  QQ `/查看模板` 与 WebUI 画廊优先读取本地文件，缺失时回退到仓库图库链接。
  安装的自定义模板同样按 `<模板名>-demo.jpg` 规则查找。
- **模板内部引用的外部资源**（背景图、装饰图等）建议：
  - 随模板一起打包进目录（相对路径），社区模板适用；
  - 或引用**长期稳定**的公开图库链接；不要使用会过期/失效的临时链接。
- 参考实现：`HatsuneMiku/` 将装饰图放在 `assets/` 下并通过图库链接引用。
