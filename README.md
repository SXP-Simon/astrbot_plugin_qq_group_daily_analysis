...
插件会在每次正常启动时记录当前版本、配置结构和模板基线。后续只要检测到**配置结构发生变化**，就会把上一次正常加载的插件配置快照保存到插件数据目录的 `config_backups`；文件名包含旧版本和备份时间，只保留最新二十份。插件版本仅用于标识备份来源，不是备份条件；调整描述或收纳配置项不会生成备份。日志会显示旧版本、备份文件名和完整路径。通过 WebUI 修改配置后，请正常重载插件或重启 AstrBot，使该配置成为下一次升级可保护的快照。
