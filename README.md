...
| 网页日报（web） | 先部署 Worker，再在插件配置页的“网页日报设置”里开启并填写地址/token。 | 输出格式需设为 web |
...
1. 点击按钮部署 `worker` 子目录。
2. 在 Cloudflare 部署页里填写 `UPLOAD_TOKEN`，并按页面提示创建或选择 `REPORTS` KV。
3. 回到插件配置页：
   - `基础设置 -> 输出格式` 设为 `web`
   - `网页日报设置 -> 启用网页日报` 打开
   - 填入 `Worker 地址` 和同一个 `UPLOAD_TOKEN`

Worker 构建时会自动从主仓库拉取模板；主仓库模板更新后，重新构建一次 Worker 即可同步。
