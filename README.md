...
[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/clown145/astrbot_plugin_qq_group_daily_analysis/tree/feat/python-worker-json-render)

1. 点击按钮部署 Worker。
2. 在 Cloudflare 部署页里设置密钥 `UPLOAD_TOKEN`（`REPORTS` KV 一般会自动创建）。
3. 回到 AstrBot WebUI：
   - `基础设置 -> 输出格式` 设为 `web`
   - `网页日报设置 -> 启用网页日报` 打开
