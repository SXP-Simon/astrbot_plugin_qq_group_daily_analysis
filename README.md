...
- 官方群事件不提供成员昵称：图片/HTML 报告隐藏昵称和 openid，仅显示头像；文本报告使用 `<@member_openid>` 艾特成员，不展示裸 openid。
- QQ 官方文本报告使用自定义 Markdown，并默认通过 AstrBot T2I 生成透明背景的 24 小时活跃直方图。可在 `QQ 官方机器人` 配置组关闭；渲染失败时自动回退为文字条形图。
- Markdown 直方图直接使用 AstrBot T2I 返回的公网 URL。请确保当前 T2I 端点域名已加入 QQ 开放平台的消息 URL 配置。
- 头像通过以下规则获取：`https://thirdqq.qlogo.cn/qqapp/{机器人AppID}/{member_openid}/640`
...
| **QQ 官方机器人** | QQ Bot API v2（WebSocket/Webhook） | 需开启群全量消息；只分析启用后实时缓存的消息，用户身份在报告中仅显示头像。 |
