...
### 📊 可视化报告
- **多种格式**: 支持图片、文本、PDF、网页链接
    - **精美图片**: 生成美观的可视化报告
    - **PDF报告**: 生成专业的PDF格式分析报告（需配置）
    - **网页链接**: 上传 JSON 到 Cloudflare Worker，群里直接发网页链接
...
| 网页日报（web） | 先部署 Worker，再在 AstrBot WebUI 的“网页日报设置”里开启并填写地址/token。 | 输出格式需设为 web |
| 自定义LLM服务 | 用户可自行选取个人提供的服务商。 | 留空则回退到默认服务商 |

## 网页日报（可选）

[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/clown145/astrbot_plugin_qq_group_daily_analysis)

1. 点击按钮部署 Worker。
2. 部署页如果默认显示 `npm run deploy`，直接继续即可，这个命令已经会走 Python Worker 的部署脚本。
3. 在 Cloudflare 部署页里设置密钥 `UPLOAD_TOKEN`（`REPORTS` KV 一般会自动创建）。
4. 回到 AstrBot WebUI：
   - `基础设置 -> 输出格式` 设为 `web`
   - `网页日报设置 -> 启用网页日报` 打开
   - 填入 `Worker 地址` 和同一个 `UPLOAD_TOKEN`

生成后的链接形如 `https://your-domain/r/<report_id>`，切模板可在后面追加 `?t=hack`、`?t=scrapbook` 这类参数。
