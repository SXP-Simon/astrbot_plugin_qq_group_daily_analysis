...
*   **📄 【历史报告与任务详情双向直达】告别“历史遗留”，指令与定时分析报告 100% 绑定任务**：
    *   **解决报告丢失归属问题**：修复以往在群内通过 `/群分析` 指令或定时自动分析生成的报告，在控制台「分析记录」详情抽屉中看不到对应报告、在「历史报告」面板中被标记为未关联“历史遗留”的缺陷。
    *   **任务抽屉直观预览**：现在打开任意一条分析任务的详情抽屉，均能直接看到并预览本次任务生成的报告图片/网页，点击即可原图查看或下载。
    *   **历史报告一键反查**：「历史报告」列表中每份日报均清晰标注对应的任务 Trace 编号，支持一键跳转至当次任务，查看该报告生成时的消息清洗量、各阶段耗时瀑布图与 Token 消耗明细。

*   **⚡ 【系统性能与健壮性底座】全面推行领域驱动设计 (DDD)，消灭隐蔽运行时异常**：
    *   **消除动态类型隐患**：全面纯化消息、统计、话题、金句与增量批次的领域数据结构，杜绝无类型字典取值带来的潜在崩溃漏洞。

## [v5.6.3] - 跨平台增量滑动窗口防越界污染、低于阈值跳过状态语义化与数据管理页面重构

*   **🛡️ 【核心修复与跨平台时间窗口】严格约束增量拉取与滑动窗口，防止陈旧游标跨天污染**：
    *   **根治停机/冷群历史数据越界总结**：解决滞后游标导致周三日报错误拉取并总结周一旧话题的问题。
    *   **跨平台适配器 DRY 架构收敛**：在 `PlatformAdapter` 基类中统一提取 `calculate_effective_start_timestamp` 与 `calculate_effective_cutoff_datetime`，将拉取起始时间强制约束在 $[now - days, now]$ 滑动窗口内，并在 `AnalysisApplicationService` 消息清洗层增加时间门禁校验。
    *   **全平台覆盖与边界容错**：OneBot、Telegram、QQ 官方与 Discord 适配器统一切换至基类算法，自动处理 Windows CRT 负时间戳边界异常。
    *   **单元测试全覆盖**：补充多平台适配器在陈旧游标跨天场景下的自动截断与历史拉取测试、滑动窗口过滤与防泄漏集成测试。

*   **🔔 【调度可观测性与交互体验】低于阈值与跳过状态语义化收敛，告别冷硬英文报错**：
    *   **规范化 `skipped (已安全跳过)` 状态**：将发言数低于最小阈值 (`below_threshold`)、无新消息 (`no_messages`)、机器人未就绪 (`bot_not_ready`)、群聊禁言 (`muted`)、名单移除 (`target_removed`) 等正常业务跳过从 `failed (失败)` 状态中剥离，在任务追踪层规范标记为 `skipped`。
    *   **详尽人机友好中文提示**：全面消除 `Auto analysis skipped: below_threshold` 等冷硬英文报错，输出如「群聊有效发言数（X 条）未达到设定的自动分析阈值（Y 条），已安全跳过本次日报生成」等详尽上下文。
    *   **WebUI 状态与详情呈现升级**：`StatusTag` 与 `TimelinePicker` 新增青色「已跳过」徽标与时序节点，不再误标为红色的「失败」；Trace 详情抽屉对已跳过任务采用蓝色 Info 通知卡片呈现，移除红色「分析过程发生异常」报警与无效的 Checkpoint 续跑按钮；筛选栏支持按「已安全跳过」快速过滤。

*   **🖥️ 【WebUI 架构重构】重构数据管理页面，修复结构化锐评白屏**：
    *   **结构化锐评崩溃修复**：修复当 `chat_quality_review` 为结构化对象（包含维度百分比及评语）时直接作为 ReactNode 渲染导致 React 抛出 `Objects are not valid as a React child` 全页白屏崩溃的问题，优雅排版各维度评分。
    *   **表格列宽溢出优化**：增量批次表格与快照表格增加横向滚动宽度（`scroll.x = 1000px`），ID 列增加 14 位截断显示、Tooltip 完整悬浮及一键复制。

*   **🌐 【静态资源维护】全项目 CDN URL 常量结构化集中提取**：
    *   将所有 HTML 报告模板、命令服务、静态素材预设中分散硬编码的 CDN URL 提取为结构化常量，便于日后统一维护与镜像源一键集中切换。
