...
*   **✨ 免 Token 切换视觉主题重新渲染**：
    *   持久化保存每日分析产物快照（Checkpoint），控制台历史报告支持一键切换视觉主题模板（手账、亚托莉、蔚蓝档案、初音未来、黑客赛博、复古未来、极简等）并秒级重新生成长图或 HTML 报告，无需重复调用大模型，消耗 0 Token。
*   **✨ 群漫画全链路追踪与日志标签**：
    *   群漫画生成流程全链路接入 Trace 追踪，支持在控制台查看分镜提示词提取（`COMIC_STORYBOARD`）与绘图投递（`COMIC_DRAWING`）阶段的耗时、角色方案、参考图数量及生图状态。
    *   插件专属日志过滤栏新增「群漫画」分类标签，方便一键筛选群漫画相关日志。
*   **✨ 大模型分析子模块启用状态与容错处理**：
    *   任务执行详情准确标注话题总结、群友画像、精彩金句与质量锐评等各子模块的开启与关闭状态。
    *   未开启某些大模型功能时正常标注「未启用」状态，不再误报为模型调用异常；全模块关闭时清晰给予说明提示。
*   **🛠️ 绘图服务与本地部署兼容优化**：
