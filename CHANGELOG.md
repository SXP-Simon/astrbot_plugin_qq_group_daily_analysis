...
*   **✨ 漫画角色方案**：使用 AstrBot 原生 `template_list` 管理多个可收起的漫画角色方案。每个方案可分别绑定漫画专用人格和原生上传的 `jpg/jpeg/png/webp` 参考图，图生图使用该方案最后加入的一张图片。
*   **🎲 每日角色随机**：可选开启每天随机切换角色。插件会在运行环境时区的当天首次生成漫画时选择一个已启用方案，并持久化当天选择，重启后保持一致；优先读取 `TZ` 环境变量，未设置或无效时使用系统本地时区。关闭时固定使用第一个已启用方案。
*   **🛠️ 配置迁移与收纳**：旧版全局 `drawing_reference_image` 自动迁移到“默认角色方案”，迁移前配置备份到插件数据目录的 `config_backups`。绘图尺寸、格式、重试和代理等低频项收纳进 WebUI 的“更多配置”。
...
*   **🛠️ T2I 视口兼容**：图片报告显式指定 `1440x900` 初始视口，避免新版 AstrBot T2I 对 `width=device-width` 无法推断固定宽度时回退到 `800x720`，造成宽版模板挤压或截断；可在低频渲染配置中调整。
